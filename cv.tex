...
\textbf{\href{https://heyrevia.ai/}{Revia (Python, React, GCP, Redis, PostgreSQL) \ExternalLink}} \hfill Jan 2024 -- Present \\
\begin{zitemize}
    \item Reduced voice chat e2e latency to an industry-leading 500ms by adopting the architecture of self-driving cars along with a multi-agent system with heavy caching.
    ...
    \item Implemented restful APIs for businesses like making calls, loading replays, CRUD operations, etc.
\end{zitemize}

\textbf{\href{https://rebyte.ai/}{Rebyte (Rust, React, GCP, Redis, PostgreSQL, Qdrant, RAG) \ExternalLink}} \hfill Nov 2023 -- Jan 2024 \\
\begin{zitemize}
    \item Investigated the latency bottleneck of the backend and located the root cause in the Deno sandbox.
    \item Extended the list of supported LLM/Embedding providers, including corresponding tokenizers.
\end{zitemize}


\textbf{\href{https://github.com/Shaunwei/RealChar}{RealChar, 6K Stars (Python, React, GCP, RAG, WhisperX, XTTS, VAD) \ExternalLink}} \hfill Aug 2023 -- Dec 2023 \\
\begin{zitemize}
    \item Major contributor for RAG, Phonecall mode, Meeting mode, Chat on pictures
    ...
    \item Built a server side VAD as a state machine based on Silero-vad and loudness. Bedrock for Phonecall mode.
\end{zitemize}

\textbf{\href{https://github.com/y1guo/auto-transcribe}{Auto-Transcribe (Python, Whisper, Demucs, Multi-process) \ExternalLink}} \hfill May 2023 -- Aug 2023 \\
\begin{zitemize}
    \item Created a software to automatically transcribe videos, including stem separation and speech recognition.
    \item 30X real-time speed, 24/7 unattended transcription with alignment. No existing products on the market.
    \item Built a WebUI to search for audio segments by transcript among 10M sentences within 1 sec.
    \item 24/7 robustness, with crash recovery and multi-GPU support. Transcribed 50TB video within a month.
    \item Used it to create video content, resulting in 680K+ views and 1.4K+ engaged subscribers.
\end{zitemize}
